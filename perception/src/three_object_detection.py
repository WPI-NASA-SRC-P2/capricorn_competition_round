...
def initObjectDetection(path_to_model, path_to_label_map):
    ...
    while not rospy.is_shutdown():
        if(g_scout_disparity is not None and g_scout_image is not None and g_excavator_disparity is not None and g_excavator_image is not None and g_hauler_disparity is not None and g_hauler_image is not None):
            detectionAlgorithm(category_index, model_fn, g_scout_image, g_scout_disparity, scout_img_pub, scout_objects_pub, scout_name)
            detectionAlgorithm(category_index, model_fn, g_excavator_image, g_excavator_disparity, excavator_img_pub, excavator_objects_pub, excavator_name)
            detectionAlgorithm(category_index, model_fn, g_hauler_image, g_hauler_disparity, hauler_img_pub, hauler_objects_pub, hauler_name)
        update_rate.sleep()
