...
{
    ROS_INFO_STREAM("[STATE_MACHINES | hauler_state_machine.cpp | " << robot_name_ << "]: " << robot_name_ << " State Machine: Going Back to Goal (High Level Vision Goal)");
...
{
    ROS_INFO_STREAM("[STATE_MACHINES | hauler_state_machine.cpp | " << robot_name_ << "]: " << robot_name_ << " State Machine: Following Excavator");
...
{
    ROS_INFO_STREAM("[STATE_MACHINES | hauler_state_machine.cpp | " << robot_name_ << "]: " << robot_name_ << " State Machine: Dumping Volatile to Processing Plant (High Level Goal)");
...
{
    ROS_INFO_STREAM("[STATE_MACHINES | hauler_state_machine.cpp | " << robot_name_ << "]: " << robot_name_ << " State Machine: Resetting Odometry at Hopper");
    return (/*undockExcavator() && */ faceProcessingPlant() && parkAtHopper() && resetOdometry() && undockHopper()); //Jibran: Got rid of GoToProcPlant as it was redundant. 
}
...
bool HaulerStateMachine::resetOdometry()
{
    ...
    srv.request.target_robot_name = COMMON_NAMES::HAULER_1_NAME;
