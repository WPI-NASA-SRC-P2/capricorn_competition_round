...
class ObjectPlotter:
    ...
    def resetOccGrid(self):
        ...
        metadata.width = int((2*self.map_dimention)/metadata.resolution) # sets the map to be 20m x 20m regardless of resolution
        metadata.height = int((2*self.map_dimention)/metadata.resolution)
        ...
        base_frame.position.x = -self.map_dimention
        base_frame.position.y = -self.map_dimention
    ...
    def addObstacle(self, obx, oby, radius):
        ...
        obx = obx + self.map_dimention
        oby = oby + self.map_dimention
