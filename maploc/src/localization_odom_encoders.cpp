...
nav_msgs::Odometry swerve_odom(const sensor_msgs::JointState::ConstPtr &joints)
{
    ...
    if(robot_name == CAPRICORN_COMMON_NAMES::HAULER + "1")
    {
        ...
        angles[2] = joints->position[5];
        angles[3] = joints->position[2];
    }
    else if(robot_name == CAPRICORN_COMMON_NAMES::EXCAVATOR + "1")
    {
        velocities[0] = joints->velocity[11] * WHEEL_RAD;
        velocities[1] = joints->velocity[14] * WHEEL_RAD;
        velocities[2] = joints->velocity[8] * WHEEL_RAD; //Back right
