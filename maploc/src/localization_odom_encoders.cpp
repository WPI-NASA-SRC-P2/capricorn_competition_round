...
class EncoderOdom
{
    public: 
         ...
         * 
         */
        int EncoderOdom(const std::string &robot_name, float update_rate) 
        {
            robot_name_ = robot_name;
            update_rate_ = update_rate;
            joint_sub_ = nh_.subscribe("/" + robot_name_ + "/joint_states", 1000, joint_state_callback);
            nav_type_sub_ = nh_.subscribe("/" + robot_name_ + COMMON_NAMES::NAV_TYPE_TOPIC, 1, nav_type_callback);
            odom_pub_ = nh_.advertise<nav_msgs::Odometry>("/capricorn/" + robot_name_ + "/odom/encoder_odom", 1, true);
            return 1;
        }
