...
int AStar::indexFromPoseStamped(geometry_msgs::Point waypoint, nav_msgs::OccupancyGrid const &oGrid)
{
  //grid coordinates
  waypoint.x = waypoint.x / oGrid.info.resolution;
  waypoint.y = std::round(waypoint.y / oGrid.info.resolution); //ROUNDING OPERATION NECESSARY - DO NOT CHAN

  //converting grid coordinates to index
  int index = (waypoint.y + (oGrid.info.height / 2)) * oGrid.info.width + (waypoint.x + (oGrid.info.width / 2));
