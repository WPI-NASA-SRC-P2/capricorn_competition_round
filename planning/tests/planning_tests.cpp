...
TEST_P(NeighborTests, GetsAllNeighborsTest)
{
  ...
  ASSERT_EQ(expectedSize, actualNeighbors.size()) << "Number of Neighbors is not as is expected";

  for (int i = 0; i < actualNeighbors.size(); ++i)
  {
    ASSERT_EQ(expectedNeighbors[i], actualNeighbors[i]) << "Neighbor at Index " << i << " is not correct expected " << expectedNeighbors[i] << " got " << actualNeighbors[i];
  }
}
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(
        ...
        std::make_tuple(12, 5, 40, 8, std::vector<int>{13, 11, 17, 18, 16, 7, 8, 6})));
