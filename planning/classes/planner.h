...
public:
  /**
   * @brief Construct a new Planner Class object
   * 
   */
  Planner();

  /**
   * @brief Destroy the Planner Class object
  ...
  std::vector<Planner::cord> findAllNeighbors(std::vector <Planner::cord> grid, int width, std::vector <Planner::cord> toSearch, int r);

  int grid_to_index(std::vector <Planner::cord>, Planner::cord);
  Planner::cord index_to_grid(std::vector <Planner::cord>, int);
  std::vector <Planner::cord> eightNeighbors(std::vector <Planner::cord>, Planner::cord, int width);
  ...
  std::vector <int> cSpace(std::vector <Planner::cord> grid, int width, std::vector <int> probibility, int threshold, int radius);
